    ...
    call this%SetTheoryParameterNumbers(16+3,last_power_index) !MMmod: DHOST 16+3 c_i (beta is not free!)
...
!            DA = Params(3)/100
    ...
    real(mcp) Params(:)
    class(TTheoryParams), target :: CMB
    real(mcp) omegam, h2, finalhubble
    !BRENT variables
    real(mcp)            :: aa,amin
    real(mcp)            :: bb,bmin
!    real(mcp), external  :: minifunc
    real(mcp)            :: value
    integer              :: status
    integer              :: stepmin
    real(mcp)            :: arg    
    logical, parameter :: mindebug=.true.
