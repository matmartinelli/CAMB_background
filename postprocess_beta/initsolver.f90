...
real*8                           :: initial_z                   !starting redshift for ODE solver
real*8                           :: final_z                     !final redshify factor for ODE solver
integer                            :: nsteps    = 10000           !number of ODE integration steps
real*8, dimension(:),allocatable :: z_ode, sol1, sol2, solH     !redshift, rho_m, Psi, H
real*8, dimension(:),allocatable :: tempz, temp1, temp2, temp3  !temporary arrays
real*8, dimension(:),allocatable :: b1, c1, d1
real*8, dimension(:),allocatable :: b2, c2, d2      !coefficient of polynomial for interpolation
real*8, dimension(:),allocatable :: bh, ch, dh
type CAMBparams
real*8 :: c2_dhost
real*8 :: c3_dhost
real*8 :: c4_dhost
...
subroutine deinterface(omm,acca,c2d,c3d,c4d,betad,diff)
      Type(CAMBparams) CP
      integer, parameter      :: n = 1                          !number of dependent variables (-1)
      real, dimension(0:n)    :: x                              !dependent variables: rho_m, Psi
      real                    :: h                              !step size
      real*8                :: rhoc_init, Psi_init            !initial conditions
      integer                 :: i,j,k
      !debugging stuff
      real*8                :: debug_a, debug_c, debug_v, first_a_debug
      real*8                :: debug_q
      real*8                :: zp, zm, dz, fplus, fminus, integral
      real*8                :: xi_dhost
      real*8                :: aini, a0
      real*8                :: diff
      real*8,intent(in)     :: omm,acca,c2d,c3d,c4d,betad
      ...
      if (9*CP%c3_dhost**2.-48*CP%c2_dhost*CP%c4_dhost-9*CP%beta_dhost*CP%c2_dhost.lt.0.d0) then
      ...
      sol1(:) = sol1(:)*2.              !Fix the missing 2 in the equations
      ...
      diff = abs(CP%H0-ispline(0.d0, z_ode, solH, bh, ch, dh, nsteps)) 
      
      if (debugging) write(*,*) 'checking obtained parameters at z=0'
      if (debugging) write(*,*) 'H(z=0) =',ispline(0.d0, z_ode, solH, bh, ch, dh, nsteps)
      if (debugging) write(*,*) 'H0     =',CP%H0
      if (debugging) write(*,*) 'Om(z=0)=',(1./3.)*ispline(0.d0, z_ode, sol1, b1, c1, d1, nsteps)
      ...
      if (debugging) then
         ...
         do i=1,nsteps
            ...
            write(747,*) z_ode(i), (1./3.)*sol1(i)*(1./((1-CP%omegav)*(1+z_ode(i))**3.+CP%omegav)), CP%omegav*(1./((1-CP%omegav)*(1+z_ode(i))**3.+CP%omegav))
            write(42,*) z_ode(i),solH(i), CP%H0*sqrt((1-CP%omegav)*(1+z_ode(i))**3.+CP%omegav)
         end do
         close(656)
         close(747)
         close(42)
...
subroutine xpsys(CP,n,k,h,x,f)
      ...
      integer n,k
      real :: h      !stepsize
      real*8 :: redshift, derivative1, derivative2, temphub
...
real*8, intent(in) :: z
integer n,k
real*8              :: rhom !dimensionless rho_matter
real*8, intent(out) :: hubble
real*8, intent(out) :: derivative1, derivative2
...
derivative1 = 3.*x(0)/(1+z) !Standard derivative for rho_m differential equation
